...
{
    assert([keyPath isEqualToString:@"events"]);
    assert(object == _runBuilder);
    self.matchedSplits = [_splitMatcher matchSplitsFromRun:_runBuilder.run withReferenceRun:_referenceRun];
}

-(void)setRunBuilder:(SSRunBuilder *)runBuilder
{
    [_runBuilder removeObserver:self forKeyPath:@"events"];
    _runBuilder = runBuilder;
    [_runBuilder addObserver:self forKeyPath:@"events" options:0 context:NULL];
}
